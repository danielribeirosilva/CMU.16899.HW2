cd % clean up the matlab environment
clear; clc; close all;
...
% Construct the quaternion that represents the body roll due to the tail
% rotor thrust
quat_pqr = quaternion_from_axis_rotation([roll_angle_trim; 0; 0;]);

% Compute the gravity vector in the "rotated" frame
g_xyz = express_vector_in_quat_frame([0; 0; g;], quat_pqr);

% collective_trim is then the sum of force due to gravity and the force
% generated by the (rate of change of)z component of the body velocity
% and the force generated by the collective control (which in this case is
% zero)
collective_trim = -( g_xyz(3) * model.params.m  + model.params.Fz(1)) / model.params.Fz(3);
...
%figure; plot(x(idx.ned,:)'); legend('north', 'east', 'down'); title('open loop'); grid on; ylim([-0.01,0.01]);
%figure; plot(x(idx.q,:)'); legend('qx', 'qy', 'qz', 'qw'); title('open loop'); grid on; ylim([-1,1]);
...
u_prev_mult = 0; u_delta_prev_mult = 1.0E03;
ned_dot_mult = 1; ned_mult = 1.0;
pqr_mult = 1; q_mult = 1.0;
always1state_mult = 1;
...
num_steps = 1000; % make sure it is sufficient for K to have converged
for i=1:num_steps
	K{i} = -inv(R + B' * Ps * B) * B' * Ps * A;
...
% cmaes_control = cmaes (target_hover_state, dt, model, idx, noise_F_T);
% 
% for i=1:1
% 	x(:,i) = target_hover_state;
% end
...
% end
% 
figure; plot(x(idx.ned,:)'); legend('north', 'east', 'down'); title('K_{ss} hover with no latency');
figure; plot(x(idx.q,:)'); legend('qx', 'qy', 'qz', 'qw'); title('K_{ss} hover with no latency');
figure; plot(x(idx.u_prev,:)'); legend('aileron','elevator','rudder','collective'); title('K_{ss} hover with no latency');
